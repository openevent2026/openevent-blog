...
要把事件中枢做稳，先要把系统里的基本对象定义清楚。OpenEvent 没有直接照搬消息队列或 RPC 的现成概念，而是围绕多模块协作中最基本的问题，抽象出四个对象：动作相关主体、协作场景、通信协议、事件详情。

- 动作相关主体 Principal：Principal 是系统中的身份标识，可以代表用户、某个 Agent、某个独立模块进程，或者某个系统账号。OpenEvent 不关心这个身份背后的业务含义，只根据身份标识执行确定的收发动作。
- 协作场景 Channel：Channel 是一个带元数据的局部协作域。每个 Channel 有自己的 channel_id、name、visibility、members，也有 protocol 来标记这个协作域内部遵循的约定。换句话说，Channel 定义的是一批事件在谁的参与下、在哪个范围内流动。和简单的消息分桶相比，它更接近一个带访问控制的协作场所。
- 通信协议 Protocol：在 OpenEvent 里，Protocol 是一个标签，每个标签指明一种基于 OpenEvent 的通信协议，用来告诉接入方：这个 Channel 里的 payload 应该按哪套上层约定来解释。除了 channel_id=0 的系统广播协议外，其他协议全部放在独立模块项目中，以可插拔方式接入。OpenEvent 只保存这个标签，不做 schema 校验，也不要求所有模块都能识别所有协议。它标记的是一门语言，中枢并不因此掌握这门语言。
- 事件详情 Message：Channel 是协作边界，Message 就是边界里真正流动的最小事实单元。一条 Message 包含 seq、channel_id、principal 和 payload。每个 OpenEvent 实例维护一条有序队列，通过 seq 标记事件在全局队列中的顺序，channel_id 指明它属于哪个协作域，principal 表明是谁发出的，payload 承载真正的业务内容。OpenEvent 的很多设计，最终都是围绕如何让 Message 被可靠写入、可控读取、准确追踪展开的。

把这四个概念串起来看，Principal 定义动作主体，Channel 定义协作边界，Protocol 定义边界内部的解释约定，Message 则是沿着这个边界流动的事实载体。OpenEvent 想稳定下来的，正是这套主体、边界、约定和事实之间的关系。
...
这样，系统真正被统一的是顺序、ACL、读写语义和传输骨架；真正被解耦的是各模块在各自 Channel 内部使用的业务语言。IM 走 im.v1，模型代理走 llm.v1，未来工具调用走 tool.v1——它们不需要被压扁成一套全局统一的 payload 模型。新增模块时，不必回头修改一个越来越臃肿的中心协议，直接开一个新的 Channel，挂上自己的协议标签即可，变化被限制在局部。
...
会话仍然存在，只是放到了协议层。OpenEvent 本体仍然不需要理解聊天业务，它只需要保证这几类消息都能按统一顺序进入同一个 Channel，并继续受同一套 ACL 和读写语义约束。至于 im.v1 字段怎么长、动作和记录怎么关联，这些属于独立 IM 模块的职责，不属于 OpenEvent 主体的内建能力。
...
Model Proxy 基于 llm.v1 协议，消费 infer.request，调用 OpenAI-compatible API，再把 infer.result 写回同一个模型 Channel。对 OpenEvent 主体来说，这只是一条请求事件和一条结果事件。模型服务可以替换，协议可以演进，但中枢和大部分模块不受影响。

Agent Demo 则是把上述模块串起来的最小闭环。它提供了一个聊天机器人，以及一套尽量简单的运行脚本，用来快速体验整套架构。
...
- IM 模块：[https://github.com/openevent2026/openevent-modules-im.git](https://github.com/openevent2026/openevent-modules-im.git)
...
项目地址：[https://github.com/openevent2026/openevent-view](https://github.com/openevent2026/openevent-view)
